...
public class CreateTaskController<Note> implements Initializable {
	...
	public <Actionevent> void saveButton(Actionevent event) {
	}
	...
	public void saveTask(ActionEvent event) {
		...
		LocalDateTime startpoint = this.startDate.atTime(INIT_VALUE_START_HOURS, INIT_VALUE_START_MINUTES);
		LocalDateTime endpoint = this.endDate.atTime(INIT_VALUE_END_MINUTES, INIT_VALUE_END_MINUTES);

		boolean regularlyOnOff = this.regularOnOff.isSelected();
		int regularType;
	...
	int regularID = creator.Creator.getRegularlyID();
	String descriptionText = this.description.getText();
	int notesPinned = 0;
	ArrayList<note.Note> notesLink = null;
	boolean autoSort = this.autoSortOnOff.isSelected();
	int duration = Integer.valueOf(hoursField.getText())*60+Integer.valueOf(minutesField.getText());
	boolean done = false;
	...
	boolean floating = false;
		try {
			creator.Creator.createTask(titleText, filepath, startpoint, endpoint, allDay, regularlyOnOff, regularType,
					regularlyID, descriptionText, alarmOnOff, alarmTime, notesPinned, notesLink, floating, autoSort,
					duration, done, periodStart, periodEnd);
		} catch (CloneNotSupportedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	
		// This method saves the current Note and it's title as a category if a category
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		minutesField.textProperty().bindBidirectional(durationSlider1.valueProperty(),
